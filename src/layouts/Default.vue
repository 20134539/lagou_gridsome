<template>
    <div>
        <section class="page-header" :style="'background-image: linear-gradient(120deg, '+$static.config.edges[0].node.backgroundColorLeft+', '+$static.config.edges[0].node.backgroundColorRight+');color: '+$static.config.edges[0].node.fontColor+';'">
            ...
            <h1 class="project-name">{{$static.config.edges[0].node.blogTitle}}</h1>
            <h2 class="project-tagline">{{$static.config.edges[0].node.blogDescribe}}</h2>
            <a :href="'https://github.com/'+$static.config.edges[0].node.githubUsername" class="btn" target="_blank">GitHub主页</a>
            <a href="https://github.com/GitHub-Laziji/vblog" class="btn" target="_blank" v-if="!$static.config.edges[0].node.mini">博客源码</a>
        </section>
                ...
                <el-row>
                    <el-col :span="10">
                        <el-menu @select="selectTopbar" :default-active="topbar.active" mode="horizontal" menu-trigger="click">
                            ...
                            <el-submenu index="#webSites" v-if="$static.config.edges[0].node.webSites.length>0">
                                <template slot="title">其他网站</template>
                                <el-menu-item :index="'#webSites-'+index" v-for="(item,index) in $static.config.edges[0].node.webSites" :key="'#webSites'+index">{{item.name}}</el-menu-item>
                            </el-submenu>
                        </el-menu>
                    </el-col>
                    <el-col :span="8" style="text-align: center;padding: 12px 0px 0px 10px">
                        ...
                        <audio ref="music" loop autoplay v-if="$static.config.edges[0].audioAutoPlay">
                            <source :src="$static.config.edges[0].node.audioUrl" type="audio/mpeg">
                        </audio>
                    ...
                    <el-col :span="4" style="text-align: right;">
                        <div style="font-size: 20px;color:#606266;margin-top: 5px">
                            <b>{{$static.config.edges[0].node.githubUsername}}</b>
                        </div>
                        <div style="color:#606266;">
                            <i class="el-icon-location"></i>&nbsp;{{$static.config.edges[0].node.location?$static.config.edges[0].node.location:'未填写地址'}}
                            <br>
                        </div>
                    </el-col>
                    <el-col :span="2" style="text-align: center;">
                        <img v-popover:bigAvatar :src="$static.config.edges[0].node.avatarUrl" style="margin-top: 4px;margin-right: 10px;width:52px; height:52px; border-radius:5px; border: 1px solid #EBEEF5"
                        />
                        <el-popover ref="bigAvatar" placement="top-start" :title="$static.config.edges[0].node.githubUsername" width="200" trigger="hover">
                            <i class="el-icon-star-on"></i>&emsp;{{$static.config.edges[0].node.name}}
                            <br>
                            <i class="el-icon-location"></i>&emsp;{{$static.config.edges[0].node.location}}
                            <br>
                            <img :src="$static.config.edges[0].node.avatarUrl" style="width: 200px;height: 200px;">
                        </el-popover>
                    </el-col>
                </el-row>
    ...
    export default {
        ...
        methods: {
            selectTopbar(index) {
                ...
                switch (index) {
                    case "#githubHome":
                        window.open('https://github.com/' + this.$static.config.edges[0].node.githubUsername)
                        break
                    case "#blog":
                        if (this.blog) {
                    ...
                    default:
                        if(/#webSites-\d+/.test(index)){
                            ...
                            let url = this.$static.config.edges[0].node.webSites[i].url
